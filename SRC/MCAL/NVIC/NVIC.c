...
NVIC_ErrorStatus NVIC_EnableIRQ(NVIC_IRQn_Type NVIC_IRQn)
{
    ...
    uint32_t NVIC_local_var,NVIC_local_REG_num ,NVIC_local_bit_num ;
...
NVIC_ErrorStatus NVIC_ClearPendingIRQ(NVIC_IRQn_Type NVIC_IRQn)
{
    ...
    if(NVIC_IRQn >= 0 && NVIC_IRQn<=_NVIC_IRQn)
    {
         NVIC_local_var  = NVIC_REG_Ptr->NVIC_ICPRx[NVIC_IRQn/32];/*divide to determine reg number*/
         NVIC_local_var |= (1<<(NVIC_IRQn%32));/*remainder to determine bit number*/
...
NVIC_ErrorStatus NVIC_GetPendingIRQ(NVIC_IRQn_Type NVIC_IRQn,uint32_t *NVIC_Active_bit)
{
    NVIC_ErrorStatus NVIC_State= NVIC_IRQNUM_NOK;
    uint32_t NVIC_local_var;
    ...
    if(NVIC_IRQn >= 0 && NVIC_IRQn<=_NVIC_IRQn)
    {
         NVIC_local_var   = NVIC_REG_Ptr->NVIC_IABRx[NVIC_IRQn/32];/*divide to determine reg number*/
         NVIC_local_var   = ((NVIC_local_var>>(NVIC_IRQn%32))&1);/*remainder to determine bit number*/
...
NVIC_ErrorStatus NVIC_SetPriority(NVIC_IRQn_Type NVIC_IRQn, uint8_t NVIC_Priority)
{
    ...
    if((NVIC_IRQn >= 0 && NVIC_IRQn<=_NVIC_IRQn) && (NVIC_Priority>=0 && NVIC_Priority<=255))
    {
        ...
        NVIC_local_var |=  (NVIC_Priority<<((NVIC_IRQn%4)*8));
        NVIC_REG_Ptr->NVIC_IPRx[NVIC_IRQn/4] = NVIC_local_var;
        NVIC_State      =NVIC_IRQNUM_OK;
    }
    else
    {
...
NVIC_ErrorStatus NVIC_GetPriority(NVIC_IRQn_Type NVIC_IRQn,uint32_t *NVIC_pirority_val)
{
    ...
    if(NVIC_IRQn >= 0 && NVIC_IRQn<=_NVIC_IRQn)
    {
        *NVIC_pirority_val=(NVIC_REG_Ptr->NVIC_IPRx[NVIC_IRQn/4]>>((NVIC_IRQn%4)*8))&NVIC_MASK_PRI;  
