...
SYSTICK_ErrorStatus SYTK_START(void)
{
    SYSTICK_ErrorStatus SYSTICK_Local_status = SYSTICK_NOK;
...
{
     SYSTICK_Ptr->STK_CTRL  = STK_EDIT_REG_MASK(SYSTICK_Ptr->STK_CTRL,SYSTICK_MASK_DISABLE,SYSTICK_SET);
...
SYSTICK_ErrorStatus SYTK_STOP(void)
{
	...
	SYSTICK_Ptr->STK_CTRL = STK_EDIT_REG_MASK(SYSTICK_Ptr->STK_CTRL,SYSTICK_MASK_DISABLE,SYSTICK_CLEAR);
	SYSTICK_Local_status  = SYSTICK_OK;
...
SYSTICK_ErrorStatus SYTK_SetTimeMS(uint32_t SYSTICK_TIME_MS)
{
    ...
    if(SYTK_CHECK_RELOAD(reload_value))
    {
        SYSTICK_Ptr->STK_LOAD = STK_EDIT_REG_MASK(SYSTICK_Ptr->STK_LOAD,reload_value,SYSTICK_SET);
        SYSTICK_Local_status = SYSTICK_OK;
    }
    else
    {
